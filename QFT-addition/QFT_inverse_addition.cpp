/**********************************
 * Description: An out-of-place implementation of inverse addition
 *              using grover's on QFT added registers.
...
std::vector<std::tuple<std::string, size_t>> sort_map(
...
__qpu__ void quantumFourierTransform(cudaq::qview<> qs) {
  ...
  for (int i = 0; i < shift_len; ++i) {
    ...
    if (ENABLE_MISC_DEBUG) {
      printf("QFT PHASE %d: %lf\n", i, shifts[i]);
    }
  }
  // Apply phase shifts
...
// Driver for adder
struct runAdder {
  __qpu__ auto operator()(const long sum, const int nbits_x, const int nbits_y,
                          const int nbits_z) {
    QFTAdder add_op;
    ...
    // Iterations for grover's alg
    int n_iter = (0.785398) * sqrt(pow(2, nbits_x) * (pow(2, nbits_x)) / sum);
    for (int i = 0; i < n_iter; i++) {
      ...
      // 4. Undo addition
      cudaq::adjoint(add_op, x_reg, y_reg, z_reg);
...
void display_full_results(std::vector<std::tuple<std::string, size_t>> results,
                          long z, int nbits_x, int nbits_y, int nbits_z,
                          size_t n_printed = 5) {
  ...
  for (auto item : results) {
    ...
    // % of whole
    if (i < n_printed) {
      printf("%d + %d = %d (%lu/%lu = %.2f%%)\n", x_val, y_val, z_val, count,
             n_shots, (float)100 * count / n_shots);
      if (ENABLE_DEBUG) {
...
void run_QFT_adder(long z) {
  // Necessary # bits computed based on input values. Min 1.
  int nbits_x = ceil(log2(max(std::vector<long>({z, 1})) + 1));
  int nbits_y = nbits_x;
  ...
  printf("\nEXPECTED VALUES\n");
  printf("[0, z]\n");
  printf("Adding values: ? + ? = %ld\n", z);

  // Draw circuit and view statevector
  if (ENABLE_CIRCUIT_FIG)
    std::cout << cudaq::draw(runAdder{}, z, nbits_x, nbits_y, nbits_z);
  if (ENABLE_STATEVECTOR) {
    auto state = cudaq::get_state(runAdder{}, z, nbits_x, nbits_y, nbits_z);
    state.dump();
  }
  ...
  auto counts =
      cudaq::sample(n_shots, runAdder{}, z, nbits_x, nbits_y, nbits_z);
  ...
  std::vector<std::tuple<std::string, size_t>> results =
      sort_map(counts.to_map());
  printf("\nMEASURED RESULTS\n");
  display_full_results(results, z, nbits_x, nbits_y, nbits_z, 5);
  printf("\n");
}
...
int main(int argc, char* argv[]) {
  ...
  printf("Usage: ./inverse_add.x [search value (z)]\n");
  long z = 13;
  if (argc >= 3) {
    z = strtol(argv[1], nullptr, 10);
  }
  printf(
      "This will attempt to use Grovers on QFT add to compute ? + ? = z "
      "(Out-of-place).\n");
  run_QFT_adder(z);
}
