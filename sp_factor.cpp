 ...
 * @note Prime factorization re-expressed as finding the integers a, b which
 * satisfy M = f(a, b) with f(a, b) = 6(a + 1)(b + 1) + s(b + 1) + sS(a + 1) − 1
 */
void calculateSemiPrimes(long N) {
  // PARSE INPUT VALUES
  // Default search value
  long search_sum = 0b1111;
  // if (argc >= 2) {
  //   if (is_numeric((std::string) argv[1])) {
  //     search_sum = strtol(argv[1], nullptr, 10);
  //   } else {
  //     printf("Search value must be given as decimal\n");
  //   }
  // } else {
